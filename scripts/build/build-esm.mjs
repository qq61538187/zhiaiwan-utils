...
import { ARTIFACT_IDENTIFIER, ESM_DEFAULT_FILE, ESM_ENTRY_FILE } from "./artifact-config.mjs";
...
for (const groupName of GROUP_EXPORTS) {
	...
	await writeFile(
		...
		[
			...
			`const ${groupName} = {`,
			...methodNames.map(
				(name, index) => `  ${name}${index === methodNames.length - 1 ? "" : ","}`,
			),
			"};",
			"",
...
await writeFile(
	...
	[
		...
		...GROUP_EXPORTS.map((groupName) => `import ${groupName} from './${groupName}.js';`),
		"",
